...
submetered_homes_feedback = np.array([140, 144, 145, 146, 151, 153, 154, 157, 159, 161])
...
for i, algo in enumerate(["CO", "FHMM", "Hart"]):
    ...
    """
    ax[i].scatter(df["num_defrost_per_day"],df["defrost_percentage"])
    ax[i].axhspan(df["defrost_percentage"].median(), df["defrost_percentage"].median())
    ax[i].axvspan(df["num_defrost_per_day"].median(), df["num_defrost_per_day"].median())
...
ax[0].set_ylabel(r"Defrost energy $\%$")
plt.tight_layout()

plt.savefig("../../figures/fridge/disag_defrost_energy_ratio.png")
plt.savefig("../../figures/fridge/disag_defrost_energy_ratio.pdf")
plt.show()
"""
plt.clf()
latexify()
